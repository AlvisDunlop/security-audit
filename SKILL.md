...
- **Read鈥憃nly first**: prefer non鈥慸estructive commands (status, ls, cat, ss, systemctl, journalctl, ps).
...
1. If config is readable, check gateway bind/mode/auth settings:
   ...
   - If config file path is known (e.g., `~/.openclaw/config.json`), read it **read鈥憃nly**.
2. Flag if:
...
1. If Control UI is present, determine whether it accepts a gatewayUrl parameter and auto鈥慶onnects.
...
### 6) Skills & Supply鈥慍hain Risk Review
...
2. Identify world鈥憆eadable or group鈥憆eadable secret files:
...
1. Review gateway logs (read鈥憃nly):
...
- **Publicly exposed gateway/UI** 鈫?bind to localhost, firewall, require auth, reverse鈥憄roxy with proper trusted proxies.
- **Old vulnerable versions** 鈫?upgrade to latest release, rotate tokens, invalidate sessions.
- **Unsafe exec policy** 鈫?require approvals, limit tools to sandbox, drop root privileges.
- **Plaintext secrets** 鈫?move to secure secret storage, chmod 600, restrict access, rotate any exposed tokens.
- **Untrusted skills** 鈫?remove, audit contents, only install from trusted authors.
